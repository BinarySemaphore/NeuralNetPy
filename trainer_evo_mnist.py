    ...
    for connection in layer.weights:
        for n_index in range(len(connection)):
            if weight_mutation_type >= 0.9:
                connection[n_index] += (int(random() * 200000) - 100000) * 0.00000001#0.0001
            elif weight_mutation_type >= 0.45:
                connection[n_index] += (int(random() * 20000) - 10000) * 0.00000001#0.0001
            else:
                connection[n_index] += (int(random() * 2000) - 1000) * 0.00000001#0.0001
    for b_index in range(len(layer.biases[0])):
        mutate_type = random()
        if bias_mutation_type >= 0.9:
            layer.biases[0][b_index] += (int(random() * 200000) - 100000) * 0.00000001#0.0001
        elif bias_mutation_type >= 0.45:
            layer.biases[0][b_index] += (int(random() * 20000) - 10000) * 0.00000001#0.0001
        else:
            layer.biases[0][b_index] += (int(random() * 2000) - 1000) * 0.00000001#0.0001
...
def init():
    global generations_per_data_set, images_per_set, labels, small_images, evo
    ...
    #bitmap_image_data(small_images, cut_off=80)
    
    seed_network = Network(n_inputs=196, layer_config=[128, 64, 32], n_outputs=10, output_activation='softmax')
...
@inlineCallbacks
def main():
    global generations_per_data_set, images_per_set, labels, small_images, evo, start_time
    ...
    while(True):
        ...
        if evo.generation % generations_per_data_set == 0:
            ...
            for image in images:
                small_images.append(md.shrink(image))
            #bitmap_image_data(small_images, cut_off=80)
            evo.replace_data(small_images, labels)
