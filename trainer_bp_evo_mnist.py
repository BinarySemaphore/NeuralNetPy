    ...
    for connection in layer.weights:
        for n_index in range(len(connection)):
            if weight_mutation_type >= 0.9:
                connection[n_index] += (int(random() * 200000) - 100000) * 0.000000001#0.0001
            elif weight_mutation_type >= 0.45:
                connection[n_index] += (int(random() * 20000) - 10000) * 0.000000001#0.0001
            else:
                connection[n_index] += (int(random() * 2000) - 1000) * 0.000000001#0.0001
    for b_index in range(len(layer.biases[0])):
        mutate_type = random()
        if bias_mutation_type >= 0.9:
            layer.biases[0][b_index] += (int(random() * 200000) - 100000) * 0.000000001#0.0001
        elif bias_mutation_type >= 0.45:
            layer.biases[0][b_index] += (int(random() * 20000) - 10000) * 0.000000001#0.0001
        else:
            layer.biases[0][b_index] += (int(random() * 2000) - 1000) * 0.000000001#0.0001
...
def init():
    ...
    generations_per_data_set = 10
    images_per_set = 200

    labels, images = md.load(start=0, end=images_per_set)
    
    seed_network = Network.load("SavedNetworks/dr_net/trained_r3_91.json")
